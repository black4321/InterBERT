...
from bertmodel.task_utils import LoadDatasets, LoadLosses, ForwardModelsTrain, ForwardModelsVal
from bertmodel.optimization import BertAdam, Adam, Adamax
...
def main():
    ...
    parser.add_argument(
        "--layer_wise_lr", action="store_true", help="whether to use the layer-wise learning rate."
    )
    parser.add_argument(
    ...
    from bertmodel.modules import InterBertForVLTasks
    ...
    model = InterBertForVLTasks.from_pretrained(
    ...
    if args.layer_wise_lr:
        interbert_weight_name = json.load(open("config/" + "param.json", "r"))

    for key, value in dict(model.named_parameters()).items():
        if value.requires_grad:
            if args.layer_wise_lr:
                if key in interbert_weight_name:
                    lr = 1e-7
                else:
                    lr = args.learning_rate
            else:
    ...
    if args.lr_scheduler == 'automatic':
        ...
        lr_reduce_list = np.array([12, 16, 20, 22, 26])
    ...
    for epochId in tqdm(range(args.num_train_epochs), desc="Epoch"):
        ...
        for step in range(max_num_iter):
            ...
            for task_id in task_ids:
                # print(iterId >= task_start_iter[task_id])

...
from bertmodel.datasets import ConceptCapLoaderTrain, ConceptCapLoaderVal
from bertmodel.modules import InterBertForMultiModalPreTraining, BertConfig
...
def main():
    ...
    parser.add_argument(
        "--itm_weight", default=1, type=float, help="weight for itm loss"
    )    
    parser.add_argument(
        "--text_weight", default=1, type=float, help="weight for text loss"
    )    
    parser.add_argument(
    ...
    parser.add_argument(
        "--span_mask", action="store_true" , help="whether to use span_masking."
    )
    parser.add_argument(
        "--cond_mask", action="store_true" , help="Whether to use conditional masking method."
    )
    parser.add_argument(
        "--dynamic_masking", action="store_true" , help="whether to use dynamic masking"
    )
    ...
    if args.from_pretrained:
        if args.continue_training:
            ...
            model = InterBertForMultiModalPreTraining.from_pretrained(ckpt_load_path, config)
        else:
    ...
    logger.info("  Num steps = %d", num_train_optimization_steps - finished_steps)

...
class ConvImageLayer(nn.Module):
    def __init__(self, config):
        super(ConvImageLayer, self).__init__()
        self.conv = nn.Conv1d(config.v_hidden_size, config.v_hidden_size, kernel_size=3, padding=1, groups=8)
        self.LayerNorm = BertLayerNorm(config.v_hidden_size, eps=1e-12)

    def forward(self, input_tensor):
        hidden_states = self.conv(input_tensor.transpose(-1, -2)).transpose(-1, -2)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)
        return hidden_states
...
class ConvTextLayer(nn.Module):
    def __init__(self, config, kernel_size, padding):
        super(ConvTextLayer, self).__init__()
        self.conv = self.conv = nn.Conv1d(config.hidden_size, config.hidden_size, kernel_size=3, padding=1, groups=8)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
...
class InterBertEncoder(nn.Module):
    ...
    def forward(
        ...
        for idx in range(self.config.num_hidden_layers, self.config.num_hidden_layers+self.config.t_num_hidden_layers):
...
class InterBertForMultiModalPreTraining(BertPreTrainedModel):
    """BERT model with multi modal pre-training heads.
    """

    def __init__(self, config):
        super(InterBertForMultiModalPreTraining, self).__init__(config)
        ...
        if masked_lm_labels is not None and next_sentence_label is not None and image_target is not None:
            ...
            if self.predict_feature:
                ...
                masked_img_loss = torch.sum(
                    img_loss * (image_label == 1).unsqueeze(2).float() * (next_sentence_label == 0).unsqueeze(1).unsqueeze(2).float()
                ) / max(torch.sum((image_label == 1) * (next_sentence_label == 0).unsqueeze(1)),1)
            else:
...
class InterBertForVLTasks(BertPreTrainedModel):
    def __init__(self, config, num_labels, dropout_prob=0.1, default_gpu=True):
        ...
        self.vil_prediction = Linear(config.bi_hidden_size, num_labels)
        self.vil_logit = Linear(config.bi_hidden_size, 1)
        self.vision_logit = Linear(config.v_hidden_size, 1)
        self.linguisic_logit = Linear(config.hidden_size, 1)
        self.fusion_method = config.fusion_method
        self.apply(self.init_bert_weights)
...
class SimpleClassifier(nn.Module):
    def __init__(self, in_dim, hid_dim, out_dim, dropout):
        ...
        # self.linear_in_1 = Linear(in_dim, hid_dim)
        # self.dropout = nn.Dropout(dropout, inplace=True)
        # self.linear_in_2 = Linear(hid_dim, in_dim)
        # self.linear_out = Linear(hid_dim, out_dim)
        # layers = [
        #     Linear(in_dim, hid_dim),
        #     nn.ReLU(),
        #     nn.Dropout(dropout, inplace=True),
        #     Linear(hid_dim, in_dim),
        # ]
        # self.LayerNorm = BertLayerNorm(in_dim, eps=1e-12)
        ...
        # ]
        # self.logit = nn.Sequential(*layers_out)

    def forward(self, x):
        # out = self.main(x)
        # out = self.LayerNorm(out + x)
        logits = self.main(x)
        # logits = self.linear_out(self.LayerNorm(self.linear_in_2(self.dropout(swish(self.linear_in_1(x)))) + x))
        return logits